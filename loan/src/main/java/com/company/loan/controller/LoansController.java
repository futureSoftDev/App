...
import com.company.loan.dto.ErrorResponseDto;
import com.company.loan.dto.LoanContactInfoDto;
import com.company.loan.dto.LoanDto;
import com.company.loan.dto.ResponseDto;
...
@Tag(
        name = "CRUD REST APIs for Loans in BankApplication",
        description = "CRUD REST APIs in BankApplication to CREATE, UPDATE, FETCH AND DELETE loan details"
)
@RestController
...
public class LoansController {
    ...
    @Operation(
            ...
            description = "REST API to create new loan inside BankApplication"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    @GetMapping
    public ResponseEntity<LoanDto> get(@RequestParam
                                       @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits")
                                       String mobileNumber) {
        LoanDto loanDto = loansService.get(mobileNumber);
    ...
    public ResponseEntity<LoanContactInfoDto> getContactInfo() {
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(loansService.getContactDetails());
    }
}
